...
namespace gazebo
{
  class GAZEBO_VISIBLE DvsPlugin : public SensorPlugin
  {
	...
	// new vars
	protected: ofstream myfile;
	protected: bool initialized;
	protected: double** sbuf;
	double** prev;
	protected: ros::Time time_curr;
	ros::Time time_prev;
	protected: bool save_csv;
    ...
    private: void processDelta(cv::Mat *curr_image);
    private: void fillEvent(double t, int x, int y, bool polarity, std::vector<dvs_msgs::Event>& events);
    private: void publishEvents(vector<dvs_msgs::Event> *events);
	//bool sortByTime(const std::vector<dvs_msgs::Event>& lhs, const std::vector<dvs_msgs::Event>& rhs);
  };
}
#endif
