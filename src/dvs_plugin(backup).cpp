...
*/
  ...
  void DvsPlugin::processDelta(cv::Mat *curr_image)
  {
	  //int pixel_x, pixel_y;
      double curr[300][300];
	  double theta = .1;//event_threshold;
...
// TODO ensure that max 10x
		sbuf[i][j] += theta;
		fillEvent(ros::Time::now(), i, j, 1, events);
break; // FIXME
	  }
...
break; // FIXME
  ...
  void DvsPlugin::fillEvent(ros::Time time, int x, int y, int polarity, std::vector<dvs_msgs::Event>& events)
  {
